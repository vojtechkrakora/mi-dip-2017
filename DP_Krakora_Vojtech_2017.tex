...
\acknowledgements{Děkuji svému vedoucímu Pavlovi Pivoňkovi, GWCPM za odborné vedení, pomoc a rady při zpracování této práce.}
...
\abstractCS{Diplomová práce se zaměřuje na detekci bezpečnostních rizik v~reálném čase. Využívám nástroj MS Azure Machine Learning, kam jsou odesílána Javou
	předzpracovaná data. Jsou použity algoritmy pro shlukování a detekci anomálie. Podstatou práce je detekce rizik z~logu integrační platformy v~reálném
	čase. Vytvořené řešení tyto údaje ukládá do NoSQL databáze a pomocí Google Charts prezentuje výsledky.}
\placeForDeclarationOfAuthenticity{Prague}
\keywordsCS{dolování z~textu, bezpečnostní rizika, shlukování, anomálie}
\keywordsEN{text mining, security risks, clustering, anomaly}
	...
	\section{Integrační platforma Unify}
		...
		\subsection{Komunikace na Unify}
			...
			\subsubsection{Zprávy s~frontou}
				Je-li nutné mít jistotu odeslání požadavku (zpravidla například nové objednávky), pošle se dotaz na službu s~frontou. Služba si požadavek uloží do fronty a odpoví zpět konzumentovi informací o~přijetí. Následně požadavek odešle cílovému systému. Jakmile od systému obdrží kladnou synchronní odpověď o~zpracování, vyjme požadavek z~fronty a více ho nevolá. V~opačném případě zkusí požadavek poslat po nějaké době znova. V~obrázku \ref{fig:esb_communication} část \uv{C)}.
...
\chapter{Instalace lokálního prostředí}
	...
	\section{Instalace databáze}
		\begin{enumerate}
			...
			\item V~MongoDB vytvoříme databázi \uv{jake-the-logger-test} příkazem \textit{use jake-the-logger-test}.
...
\chapter{Návod na experiment}
	...
	\end{figure}
	\begin{figure}[htb]\centering
		\tmpframe{\includegraphics[width=\textwidth]{./img/azure/experiment07}}	
		\caption{Deploy webové služby v~MS Azure ML}
		\label{fig:azure-experiment07}
	\end{figure}
	\begin{figure}[htb]\centering
		\tmpframe{\includegraphics[width=\textwidth]{./img/azure/experiment08}}	
		\caption{Informace k webové službě v~MS Azure ML}
		\label{fig:azure-experiment08}
	\end{figure}
...
\begin{figure}
	\dirtree{%
		...
		.1 data\DTcomment{experimental data}.
		.1 deployments\DTcomment{jar and war files ready for deployment}.
		.1 src\DTcomment{sources}.
		.2 thesis\DTcomment{the directory of \LaTeX{} source codes of the thesis}.
		.2 thesis\DTcomment{the directory of source codes of the application}.
		.1 text\DTcomment{the thesis text directory}.
		.2 thesis.pdf\DTcomment{the thesis text in PDF format}.
		.1 wildfly-jake-the-logger-SDK.zip\DTcomment{application server with all modules}.		
	}
\end{figure}
